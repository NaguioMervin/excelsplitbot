...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mode:
        await update.message.reply_text("⚠️ Please choose a mode first using /split, /merge, or /compare.")
        return
    ...
    if mode == "split":
        ...
        await update.message.reply_text(f"📎 Added: {update.message.document.file_name}")
        ...
        await update.message.reply_text(f"📎 Added for comparison: {update.message.document.file_name}")
...
# 🔹 SPLIT EXCEL
# ================================
async def process_split(update: Update, context: ContextTypes.DEFAULT_TYPE, file_path):
...
async def process_merge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(files) < 2:
        await update.message.reply_text("⚠️ Please send at least 2 Excel files to merge.")
        return
...
async def compare_excels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(files) != 2:
        await update.message.reply_text("⚠️ Please upload exactly 2 Excel files for comparison.")
        return
    ...
    try:
        ...
        if sorted(df1.columns) != sorted(df2.columns):
            await update.message.reply_text("❌ Columns are not identical across both files.")
            return
        df2 = df2[df1.columns]
        ...
        if comparison.all().all():
            await update.message.reply_text("✅ The two Excel files are *identical* in all data values!")
        else:
            ...
            diff_df = pd.concat([df1_sorted, df2_sorted], keys=["File1", "File2"]).reset_index()
            diff_df.to_excel("comparison_result.xlsx", index=False)
            await update.message.reply_document(document=open("comparison_result.xlsx", "rb"))
